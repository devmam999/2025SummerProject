...
export const getRouteSuggestions = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const parse = routeSchema.safeParse(req.body?.route);
    if (!parse.success) return res.status(400).json({ error: 'Invalid route payload', details: parse.error.flatten() });
...
export const refineRoute = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const parse = refineSchema.safeParse(req.body);
    if (!parse.success) return res.status(400).json({ error: 'Invalid refine payload', details: parse.error.flatten() });
